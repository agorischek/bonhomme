    ...
    fn recover_operations(
        ...
        for file in edited {
            ...
            match base_files.get(&file.path) {
                None => creates.extend(import_markdown_file(&file.path, &file.content)),
                Some(file_symbol) => recover_markdown_file(
                    base,
                    file_symbol,
                    &file.path,
                    &file.content,
...
fn recover_markdown_file(
    ...
    if metadata_string(&file_symbol.metadata, "preamble").unwrap_or_default() != preamble {
        updates.push(Operation::UpdateSymbol {
            symbol_id: file_symbol.id,
            name: None,
            body: None,
            metadata: Some(file_metadata(path, &preamble)),
        });
    }
    ...
    let base_by_name: BTreeMap<&str, &SymbolNode> = base
        ...
        .into_iter()
        .filter(|child| child.kind == SECTION_KIND)
        .map(|child| (child.name.as_str(), child))
        .collect();
    let edited_names: BTreeSet<&str> = sections
        .iter()
            ...
            Some(child) => {
                if child.body.as_deref() != Some(section.body.as_str()) {
                    updates.push(Operation::UpdateSymbol {
                        ...
                        metadata: Some(json!({ "level": section.level, "title": section.title })),
                    });
                }
            }
